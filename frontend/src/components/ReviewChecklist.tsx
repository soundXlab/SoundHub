...
export default function ReviewChecklist({ sessionId }: ReviewChecklistProps) {
  ...
  const audioChecks = checks.filter((c) => c.check_type && c.check_type in AUDIO_CHECK_TYPES);
  const otherChecks = checks.filter((c) => !c.check_type || !(c.check_type in AUDIO_CHECK_TYPES));
            ...
            {audioChecks.map((c) => (
              ...
              >
                ...
                <span style={{ flex: 1, fontSize: 13, fontWeight: 500 }}>
                  {c.check_type ? AUDIO_CHECK_TYPES[c.check_type as keyof typeof AUDIO_CHECK_TYPES] || c.check_type : "check"}
                </span>
